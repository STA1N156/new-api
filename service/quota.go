...
func PreWssConsumeQuota(ctx *gin.Context, relayInfo *relaycommon.RelayInfo, usage *dto.RealtimeUsage) error {
	...
	err = PostConsumeQuota(relayInfo, quota)
	if err != nil {
		return err
	}
...
func PostConsumeQuota(relayInfo *relaycommon.RelayInfo, quota int) error {
	_, err := postConsumeQuotaWithResult(relayInfo, quota)
	return err
}

func postConsumeQuotaWithResult(relayInfo *relaycommon.RelayInfo, quota int) (result postConsumeQuotaResult, err error) {
	...
	if relayInfo != nil && relayInfo.BillingSource == BillingSourceSubscription {
		...
		delta := int64(quota)
		if delta != 0 {
			if err := model.PostConsumeUserSubscriptionDelta(relayInfo.SubscriptionId, delta, relayInfo.StartTime.Unix()); err != nil {
				return result, err
			}
